    ...
    public class MenuController : Controller
    {
        ...
        public ActionResult Create()
        {
            try
            {
                ...
                //var pageList = Global.Sql(db, "select [ID],[Model] from [dbo].[PowerPlant_Technical]");
                var pageList = Global.Sql(db, "select [PageID],[PageTitle] from [dbo].[AspCMSPage] where [PageBody] is not null");
                ViewBag.pageList = pageList;

                var parentMenu = Global.Sql(db, "select [MenuID],[MenuName] from [dbo].[AspCMSMenu] where [MenuType] in ('Right Menu', 'Top Menu')");
